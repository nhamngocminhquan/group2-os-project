...
#define tmrTIMER_1_FREQUENCY	( 1000UL )
...
void timer1_user_callback(void) {
    ...
    sensor_read_count++;
    __asm ("MOV R2, %[input_i]"
            :  /* This is an empty output operand list */
            : [input_i] "r" (sensor_read_count)
    );
    
    // Read sensors or do slower operations here
    // Example: ADC reading, sensor polling
}
...
void main(void) {
    ...
    // Initialize system (clocks, GPIO, etc.)
    // system_init(); // Your system initialization here
    ...
    while (1) {
        ...
        // Add delay or enter low power mode
